...
- 리허설·시연: `/classobs/new` 세션 목록의 **데모** 칸에서 **가상 5명 넣기** / **가상 지우기**. 가상 학생이 든 세션은 목록에 `가상 5명` 으로 표시되고, 비교 화면에서는 이름 옆에 "가상" 배지가 붙는다. 실제 학생 기록은 건드리지 않는다
  (학생이 실수로 누르지 않도록 비교 화면에는 데모 버튼이 없다)
